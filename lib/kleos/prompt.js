...
export function buildKleosScorePrompt({ kleosData }) {
  return [
    ...
    "",
    "For dumbbell exercises, the listed estimated weight is the weight of each individual dumbbell, not the combined total.",
    "",
...
function formatStrengthMetricsForPrompt(metrics = []) {
  ...
  return [...metrics]
    ...
    .map((metric) => {
      ...
      const unit = isDumbbellExercise(metric.exercise_name) ? " KG per dumbbell" : " KG";
      const equipment = String(metric.equipment_name || "").trim() || "equipment not recorded";
      return `- ${state} — ${metric.exercise_name}: estimated 1RM ${formatNumber(metric.best_1rm)}${unit}; equipment ${equipment}; ${metric.qualifying_sessions} qualifying sessions when measured; achieved ${formatDate(metric.achieved_on)}; source Heracles; estimation basis ${metric.estimation_basis || "observed_e1rm_high"}.`;
    })
    .join("\n");
}
