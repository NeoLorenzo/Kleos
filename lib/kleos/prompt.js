...
export function buildKleosScorePrompt({ kleosData }) {
  return [
    ...
    "## Strength and Physical Capability",
    "",
    "### Heracles Strength Snapshot",
    "",
    "Heracles is the canonical source for strength-performance data. A CURRENT exercise appears only when it was trained in at least 3 distinct completed sessions across the current 30-calendar-day window. Its value is the highest positive upper observed Brzycki/Epley estimated 1RM from valid non-warm-up RIR 0-3 working sets in that window. The listed equipment is the historical machine/equipment snapshot from the exact session that produced that selected e1RM. It is an estimate, not a measured true 1RM. A STALE value is the last qualifying value retained by Kleos after the exercise stopped meeting the current-window rule.",
    "",
    "When Heracles has body-weight coverage for the exact workout date that produced the selected absolute e1RM, Kleos also receives bodyweight-relative e1RM = selected absolute e1RM / body weight on that same date. Workout-date body weight may be measured or linearly interpolated between surrounding measurements; it is never extrapolated. The relative value is context attached to the same absolute winner, not an independently selected relative-strength PR.",
    "",
    "For dumbbell exercises, the listed estimated weight is the weight of each individual dumbbell, not the combined total. Bodyweight-relative values likewise use the per-dumbbell convention.",
    "",
    formatStrengthMetricsForPrompt(kleosData.strengthMetrics),
    "",
    "### Body Metrics",
    "",
...
function formatStrengthProfileForPrompt(strengthProfile) {
  ...
  const bodyWeightMeasuredOn = strengthProfile?.bodyWeightMeasuredOn;
  ...
  if (
    ...
    lines.push(
      `- Body weight: ${formatNumber(bodyWeightKg)} KG, measured ${formatDate(bodyWeightMeasuredOn)}; source Heracles (read-only synchronized value).`
    );
  }
  ...
  return lines.length ? lines.join("\n") : "No synchronized body weight or height saved yet.";
}
...
function formatStrengthMetricsForPrompt(metrics = []) {
  if (!metrics.length) {
    return "No Heracles strength snapshot has been synced yet.";
  }

  return [...metrics]
    .sort((left, right) => {
      if (Boolean(left.is_current) !== Boolean(right.is_current)) return left.is_current ? -1 : 1;
      return String(left.exercise_name || "").localeCompare(String(right.exercise_name || ""));
    })
    .map((metric) => {
      const state = metric.is_current ? "CURRENT" : "STALE";
      const dumbbell = isDumbbellExercise(metric.exercise_name);
      const unit = dumbbell ? " KG per dumbbell" : " KG";
      const equipment = String(metric.equipment_name || "").trim() || "equipment not recorded";
      const hasRelative = metric.best_1rm_relative_bw !== null
        && metric.best_1rm_relative_bw !== undefined
        && metric.body_weight_kg_at_achieved !== null
        && metric.body_weight_kg_at_achieved !== undefined;
      const relativeContext = hasRelative
        ? `; bodyweight-relative e1RM ${formatBodyWeightMultiple(metric.best_1rm_relative_bw)}× BW${dumbbell ? " per dumbbell" : ""}, using ${formatNumber(metric.body_weight_kg_at_achieved)} KG ${metric.body_weight_kind || "resolved"} body weight on the achieved date`
        : "; bodyweight-relative e1RM unavailable because Heracles has no body-weight value for the achieved date";
      return `- ${state} — ${metric.exercise_name}: estimated 1RM ${formatNumber(metric.best_1rm)}${unit}${relativeContext}; equipment ${equipment}; ${metric.qualifying_sessions} qualifying sessions when measured; achieved ${formatDate(metric.achieved_on)}; source Heracles; estimation basis ${metric.estimation_basis || "observed_e1rm_high"}.`;
    })
    .join("\n");
}
