...
export function buildKleosScorePrompt({ kleosData }) {
  return [
    ...
    "",
    "Heracles is the canonical source for strength-performance data. A CURRENT exercise appears only when it was trained in at least 3 distinct completed sessions across the current 30-calendar-day window. Its value is the highest positive upper observed Brzycki/Epley estimated 1RM from valid non-warm-up RIR 0-3 working sets in that window. It is an estimate, not a measured true 1RM. A STALE value is the last qualifying value retained by Kleos after the exercise stopped meeting the current-window rule.",
    "",
...
function formatStrengthMetricsForPrompt(metrics = []) {
  ...
  return [...metrics]
    ...
    .map((metric) => {
      ...
      return `- ${state} — ${metric.exercise_name}: estimated 1RM ${formatNumber(metric.best_1rm)}${unit}; ${metric.qualifying_sessions} qualifying sessions when measured; achieved ${formatDate(metric.achieved_on)}; source Heracles; estimation basis ${metric.estimation_basis || "observed_e1rm_high"}.`;
    })
    .join("\n");
}
