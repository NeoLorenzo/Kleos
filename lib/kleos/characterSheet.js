...
export function buildCharacterEvidence(kleosData = {}) {
  ...
  const strengthMetrics = kleosData.strengthMetrics || [];
  const currentStrengthMetrics = strengthMetrics.filter((metric) => metric.is_current);
  const staleStrengthMetrics = strengthMetrics.filter((metric) => !metric.is_current);
  ...
  const currentStrengthEvidence = currentStrengthMetrics.map((metric) => {
    const unit = isDumbbellExercise(metric.exercise_name) ? " kg per dumbbell" : " kg";
    const equipment = String(metric.equipment_name || "").trim();
    return `${metric.exercise_name}: estimated 1RM ${formatNumber(metric.best_1rm)}${unit}${equipment ? ` on ${equipment}` : ""} (${metric.qualifying_sessions} qualifying sessions in the current 30-day window)`;
  });
  ...
  return {
    physical: compact([
      ...
      ...currentStrengthEvidence,
      currentStrengthMetrics.length
        ? `${currentStrengthMetrics.length} current Heracles strength metric${currentStrengthMetrics.length === 1 ? "" : "s"}`
        : "No exercise currently meets the Heracles 30-day / 3-session strength threshold",
      staleStrengthMetrics.length
        ? `${staleStrengthMetrics.length} last-known Heracles strength metric${staleStrengthMetrics.length === 1 ? " is" : "s are"} stale`
        : null,
