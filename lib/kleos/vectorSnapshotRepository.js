...
export async function persistVectorSnapshot(payload) {
  ...
  if (snapshot.methodologyVersion === "2.0.0") {
    throw new Error(
      "KLEOS_CURRENT_METHODOLOGY_SERVER_WRITER_REQUIRED: Methodology 2.0 snapshots must be created through the deterministic server-side evaluator contract."
    );
  }
