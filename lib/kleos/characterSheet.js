...
export function buildCharacterEvidence(kleosData = {}) {
  ...
  const currentStrengthEvidence = currentStrengthMetrics.map((metric) => {
    ...
    return `${metric.exercise_name}: estimated 1RM ${formatNumber(metric.best_1rm)}${unit} (${metric.qualifying_sessions} qualifying sessions in the current 30-day window)`;
  });
